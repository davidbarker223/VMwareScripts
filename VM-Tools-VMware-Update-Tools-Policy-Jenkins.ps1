...
# @modify date 2022-11-22 16:00:39
#
...
if ($null -ne $email){
    Send-MailMessage -To $email -From "No-Reply-Jenkins@solera.com" -Subject "VMware-Tools-Config-Update-Report-$Date" -Attachments "$ReportExport\VMware-Tools-Config-Update-Report-$Date.txt" -SmtpServer 'mail.axadmin.net' -Port 25 -WarningAction Ignore
}