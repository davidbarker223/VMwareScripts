...
if ($null -ne $email){
    Send-MailMessage -To $email -From "" -Subject "VMware-VM-Power-Status-Check-Report-$Date" -Attachments "$ReportExport\VMware-VM-Power-Status-Check-Report-$Date.txt" -SmtpServer '' -Port 25 -WarningAction Ignore
}
